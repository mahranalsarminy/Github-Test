...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['section_image']) && $_FILES['section_image']['error'] === UPLOAD_ERR_OK) {
        ...
        $file_info = pathinfo($_FILES['section_image']['name']);
        ...
        if (in_array($file_extension, $allowed_extensions)) {
            ...
            if (move_uploaded_file($_FILES['section_image']['tmp_name'], $upload_path)) {
                $image_url = '/uploads/about/' . $new_filename;
            } else {
    ...
    if (empty($title)) {
        ...
        try {
            $stmt = $pdo->prepare("
                INSERT INTO about_content (title, content, image_url, sort_order, is_active) 
                VALUES (?, ?, ?, ?, ?)
            ");
            
            $stmt->execute([$title, $content, $image_url, $sort_order, $is_active]);
...
<head>
    ...
    <title>Add About Section - Admin Panel</title>
    ...
    <div class="main-container ml-64 p-4">
        <main class="container mx-auto mt-8">
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-3xl font-semibold">Add New About Section</h1>
                <a href="about.php" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">
                    Back to About
                </a>
            </div>
                ...
                <form method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="content" id="content" 
                                  class="w-full p-2 border rounded focus:ring-2 focus:ring-blue-400"><?php echo isset($content) ? $content : ''; ?></textarea>
                    </div>
