<?php
/**
 * Features Management - Edit Feature
 * 
 * Allows administrators to edit existing features on the website.
 * 
 * @package WallPix
 * @version 1.0.0
 */
...
if (!isset($_GET['id']) || !is_numeric($_GET['id'])) {
    $_SESSION['error'] = "Invalid feature request.";
...
// Available icon colors (same as in add.php)
$availableColors = [
    'blue-500' => 'Blue', 
    'green-500' => 'Green', 
    'red-500' => 'Red', 
...
try {
    $stmt = $pdo->prepare("
        SELECT title, description, image_url, icon_class, icon_color, sort_order, is_active, created_at
    ...
    $_SESSION['error'] = "Error retrieving feature data: " . $e->getMessage();
    header("Location: index.php");
    exit;
}
    ...
    // Handle new image upload
        ...
        if (empty($errors)) {
            ...
            if (move_uploaded_file($file_tmp, $upload_path)) {
                ...
                $new_image_url = '/uploads/features/' . $new_file_name;
            } else {
    ...
    if (empty($errors)) {
        try {
            ...
            ")->execute([
                ...
                ':description' => "Updated feature: $title (ID: $feature_id)"
            ]);
        ...
        <div class="p-6 <?php echo $darkMode ? 'bg-gray-900' : 'bg-gray-100'; ?>">
            <div class="mb-6">
                ...
                <?php if ($success): ?>
                    <div class="bg-green-100 border-l-4 border-green-500 text-green-700 p-4 mb-6 <?php echo $darkMode ? 'bg-green-900 text-green-300 border-green-500' : ''; ?>" role="alert">
                        <div class="flex items-center">
                            ...
                            <div class="ml-3">
                                <p class="font-medium">Feature has been updated successfully!</p>
                            </div>
                        </div>
                    </div>
                <?php endif; ?>
                ...
                <div class="bg-white <?php echo $darkMode ? 'bg-gray-800 text-white' : ''; ?> rounded-lg shadow-md p-6">
                    <form action="edit.php?id=<?= $feature_id ?>" method="post" enctype="multipart/form-data" class="space-y-6">
                        <div>
                            ...
                            <div class="mt-2 flex items-center space-x-4">
                                <div id="image_preview" class="h-32 w-32 border rounded-md flex items-center justify-center overflow-hidden <?php echo $darkMode ? 'bg-gray-700 border-gray-600' : 'bg-gray-100 border-gray-200'; ?>">
                                    <?php if (!empty($image_url)): ?>
                                        <img src="<?= htmlspecialchars($image_url) ?>" class="max-w-full max-h-full object-contain" alt="Feature Image">
                                    <?php else: ?>
...
$currentDateTime = '2025-03-19 04:57:47';
$currentUser = 'mahranalsarminy';
